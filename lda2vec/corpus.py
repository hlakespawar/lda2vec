...
try:
    ...
    print("import of dl-dist failed")
    pass
...
class Corpus():
    ...
    def _loose_keys_ordered(self):
        ...
        specials = np.sort(list(self.specials.values()), axis=None)
        keys = np.concatenate((list(specials), list(keys)))
    ...
    def finalize(self):
        ...
        self.specials_to_compact = {s: self.loose_to_compact[self.keys_loose[i]]
                                    for i, s in enumerate(self.specials.keys())}
    ...
    def compact_to_flat(self, word_compact, *components):
        ...
        for component in components:
            raveled = np.tile(np.array(component)[:, None], max_length)[idx]
    ...
    def compact_to_bow(self, word_compact, max_compact_index=None):
        ...
        """
        if max_compact_index is None:
            max_compact_index = int(word_compact.max())

        print(max_compact_index)

        def bincount(x):
            c = Counter(x)
            ret = np.zeros(max_compact_index+1)
            for k, v in c.items():
                ret[int(k)] = v
            return ret
            # w, c = np.unique(x, return_inverse=True)
            # return np.bincount(c, minlength=max_compact_index + 1)
        axis = len(word_compact.shape) - 1
        print(word_compact.shape)
...
def fast_replace(data, keys, values, skip_checks=True):
    ...
    keys_l = list(keys.ravel())

    if not skip_checks:
        msg = "data has elements not in keys"
        assert max(data) <= max(keys_l), msg
    sdx = np.argsort(keys)
